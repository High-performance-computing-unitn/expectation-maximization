...
*/
float gaussian(float *x, float *mean, float **cov) {
    // x - mean
    float *x_u = allocate_array(D);
    for (int i = 0; i < D; i++) {
        x_u[i] = x[i] - mean[i];
    }
    ...
    float det;
    float **inv = allocate_matrix(D, D);
    inverse(cov, inv, &det);

    // multiply (x-mean) and inverse of covariance
    float *x_u_inv = allocate_array(D);
...
void e_step(float **X, float **mean, float ***cov, float *weights, float **p_val) {
    for (int i = 0; i < N; i++) { // iterate over the training examples
        ...
        float *gaussians = allocate_array(K); // store the result of gaussian pdf to avoid computing it twice
